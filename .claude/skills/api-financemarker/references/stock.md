...
| `dividend_strike`                      | int      | Сколько лет подряд эмитент **не пропускал** дивиденд (дивидендный streak).                |
...
| `dividend_gap_last` / `_average`       | int      | Размер последнего/среднего дивидендного gap в днях до закрытия.                           |
...
| `curs`                                                 | float             | Курс пересчёта (если метрика в валюте, отличной от рубля). Для рублёвых обычно `1.0`.                        |
...
- Расход `day_limit` (эмпирически, сессия 2026-05-14; в публичной документации FM модели нет):
  - Тарифицируется уникальная пара `(тикер, точная query-строка include)`, не HTTP-запрос. Повтор того же ключа — бесплатно, FM возвращает кешированный ответ (TTL ≥ нескольких минут, верхняя граница не измерена).
  - FM сравнивает строку `include` буквально: `include=summary` и `include=info,summary` считаются разными ключами, даже если payload совпадает.
  - Стоимость новой пары растёт сублинейно по числу разделов: ~1 за одиночный раздел, ~2 за 3 раздела, 3–5 за все 10. Дополнительно зависит от объёма данных тикера (длина истории отчётности, число операционных метрик).
- Канонизировать строку `include` на стороне клиента: фиксированный порядок разделов, единый формат, один use case → одна строка `include`. Это даёт максимальный hit-rate на серверный кеш FM.
- Если нужны несколько разделов одной бумаги — запрашивать одной строкой: один общий запрос с 10 разделами обходится в ~2× дешевле, чем 10 отдельных вызовов по разделу.
- Не «расширять» или «облегчать» `include` между запросами одного тикера — переход с `info,summary,ratios` на `info,ratios` оплачивается заново как новая пара.
