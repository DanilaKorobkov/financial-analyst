...
- Расход `day_limit` тарифицируется **за каждый блок данных** в `include` (по официальному ответу support FM, 2026-05-15). Запрос с N разделов в `include` спишет до N единиц.
- Запрос без `include` эмпирически возвращает все массивы пустыми (заполнен только `info`), хотя письмо FM и swagger заявляют дефолт `ratios,reports,dividends`. Расхождение не объяснено — всегда передавай `include` явно с минимально необходимым набором.
- Серверный кеш существует, но без гарантий: одинаковые повторы могут как вернуться из кеша бесплатно, так и тарифицироваться повторно. Полагаться на кеш FM при проектировании клиента нельзя — для надёжной экономии нужен локальный кеш на стороне Go-runtime.
- Ключ кеша — точная строка `include`. `include=summary` ≠ `include=info,summary` ≠ `include=summary,info` — все три считаются разными запросами, даже если payload совпадает. Канонизировать строку `include` на стороне клиента: фиксированный порядок разделов, единый формат, один use case → одна строка.
- Полные модели тарификации и кеширования с цитатами из официального письма FM — [`billing.md`](billing.md).
