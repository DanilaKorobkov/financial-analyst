...
Генерация TypeScript-клиента из `.proto` пока не настроена — типы
ответов описаны структурно в `src/api.ts`. Если контракт начнёт меняться
часто, вернёмся к `buf gen` с TS-плагинами; до тех пор структурная
типизация дешевле в поддержке.
