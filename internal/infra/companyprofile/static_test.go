...
func (s *staticSuite) TestFindByTickerReturnsConfiguredFields() {
	repo := companyprofile.NewStatic([]data.Field{company.FieldTicker, company.FieldISIN})

	got, err := repo.FindByTicker(context.Background(), "any")

	s.Require().NoError(err)
	s.Equal([]data.Field{company.FieldTicker, company.FieldISIN}, got.FieldIDs)
}
...
func (s *staticSuite) TestFindByTickerSameForAnyTicker() {
	repo := companyprofile.NewStatic([]data.Field{company.FieldTicker})
...
func (s *staticSuite) TestFindByTickerIsolatedFromExternalSlice() {
	input := []data.Field{company.FieldTicker}
	...
	s.Require().NoError(err)
	s.Equal([]data.Field{company.FieldTicker}, got.FieldIDs)
}
